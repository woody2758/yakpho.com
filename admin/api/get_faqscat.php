...
try {
    ...
    $stmt = $db->prepare("
        SELECT lang_code, faqscat_name, faqscat_detail
        FROM faqscat_translations
        WHERE faqscat_id = ?
    ");
    $stmt->execute([$id]);
    ...
    foreach ($translations as $trans) {
        $translationsObj[$trans['lang_code']] = [
            'faqscat_name' => $trans['faqscat_name'] ?? '',
            'faqscat_detail' => $trans['faqscat_detail'] ?? ''
        ];
    }
